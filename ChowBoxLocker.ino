/*
   ...
   a smartphone bluetooth controller app and allows the locker to be opened.

   Quick specs of locker box:

   -12volt connected items
   battery pack(10 AAA batteries)
   door actuator
   solenoid door lock
   led light
   ...
   The smartphone bluetooth controller app is responsible for the pairing of the HC-05 bluetooth module. The app has
   a password input field along with two buttons below it with the commands of "open door" and "close door". The user
   must input the correct password along with pushing the "open door" button in order for the locker to open. The
   "close door" button command is not used since the door can only be closed by hand. But it allows locker
   customization if a linear actuator were to be used instead. When the door is commanded open the app sends a string
   which contains a command + "=" + password  example: "open=1234"  where "open" is the command "=" is the separator
   and "1234" is the password. This program will then handle the input and determine what action to perform.
